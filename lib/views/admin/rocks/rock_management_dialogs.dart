...
void showAddEditRockSheetA(BuildContext context, {RockModels? rock}) {
...
// trong file rock_management_dialogs.dart

void showDeleteRockConfirmationDialog(BuildContext context, {required RockModels rock}) {
  showDialog(
    context: context,
    builder: (dialogContext) => AlertDialog(
      title: Text('user_management.delete_confirmation_title'.tr()),
      content: Text('Bạn có chắc muốn xóa đá "${rock.tenDa ?? ''}"?'),
      actions: [
        ...
        TextButton(
          onPressed: () async {
            ...
            try {
              await viewModel.deleteRock(rock);

              // SỬA: CHỈ POP DIALOG, KHÔNG POP MÀN HÌNH CHÍNH
              Navigator.pop(dialogContext);

              messenger.showSnackBar(SnackBar(content: Text('rock_management.delete_success'.tr()), backgroundColor: Colors.green));
            } catch (e) {
              Navigator.pop(dialogContext);
              messenger.showSnackBar(SnackBar(content: Text('rock_management.delete_failed'.tr(namedArgs: {'error': e.toString()}))));
            }
          },
...
class _AddEditRockFormState extends State<AddEditRockForm> {
  ...
  Future<void> _handleSave() async {
    ...
    final rockData = RockModels(
      uid: widget.isEditMode ? widget.rock!.uid : '',
